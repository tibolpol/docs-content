...
Docker is a platform as a service (PaaS) tool, that uses OS-level virtualization to deliver software in packages called containers.
...
3. Use the following command to pull/run the Docker container:
  ```
  docker pull <insert command here>
  ```

<Message type="tip">

Refer to the [pipenv documentation](/compute/gpu/how-to/use-pipenv/) for how to use the pipenv virtual environment inside the container.
...
[Block Storage](/storage/block/quickstart/) volumes are independent from your GPU instance and provide three-time replicated storage. It is recommended to use Block Storage for storing your datasets, training logs, model source code, etc...
...
The default behavior of Docker when running a container using `docker run`, is not publish any internal ports of the container to the outside world. To access services on a container outside of Docker, you have to map the containers internal ports using the `--publish` or `-p` flag.
